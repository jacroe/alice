...
function alice_deluge_addLocal($file)
{
	$curl = curl_init();
	curl_setopt($curl, CURLOPT_URL, DELUGE_SERVER);
	curl_setopt($curl, CURLOPT_POSTFIELDS, '{"method":"web.add_torrents","params":[[{"path":"'.$file.'","options":null}]],"id":1}');
...
function alice_deluge_addWeb($url)
{
	$curl = curl_init();
	curl_setopt($curl, CURLOPT_URL, DELUGE_SERVER);
	...
	return alice_deluge_addLocal(json_decode($data)->result); 
}
